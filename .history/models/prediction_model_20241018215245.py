...
def train_and_predict_with_predicted_causes(csv_file):
    ...
    for index, row in predicted_data.iterrows():
        # Sử dụng DataFrame với tên cột khớp với những gì mô hình đã học
        features = pd.DataFrame([row[['global_temperature', 'forest_cover', 'co2_emissions', 'climate_impact']]], 
                                columns=['global_temperature', 'forest_cover', 'co2_emissions', 'climate_impact'])

        # Kiểm tra dữ liệu đầu vào của mỗi năm để chắc chắn rằng nó khác nhau
        print(f"Year: {row['year']}, Features: {features}")
