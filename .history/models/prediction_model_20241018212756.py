...
def train_and_predict_with_predicted_causes(csv_file):
    ...
    for index, row in predicted_data.iterrows():
        # Lấy các biến đầu vào cho năm hiện tại dưới dạng DataFrame với tên cột
        features = pd.DataFrame([row[['global_temperature', 'forest_cover', 'co2_emissions']]], columns=['global_temperature', 'forest_cover', 'co2_emissions'])
